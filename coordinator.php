...
<head>
    ...
    <style>
        ...
        @keyframes morph-blob-2 {
            ...
            50% { border-radius: 42% 58% 70% 30% / 45% 45% 55% 55%; transform: translate(-60px, -40px) scale(1.1); }
        }
...
<body class="relative min-h-screen bg-slate-950 text-slate-100 overflow-x-hidden">
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none z-[-2]">
        <div class="absolute top-[5%] left-[10%] w-[35vw] h-[35vw] max-w-[500px] bg-emerald-500/10 blur-[100px] animate-blob-1"></div>
        <div class="absolute bottom-[10%] right-[10%] w-[40vw] h-[40vw] max-w-[600px] bg-blue-600/10 blur-[120px] animate-blob-2"></div>
    </div>
        ...
        <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col">
                    <span class="text-lg font-bold tracking-wide uppercase bg-gradient-to-r from-emerald-400 to-blue-400 bg-clip-text text-transparent">Coordinator Panel</span>
                    <span class="text-[10px] text-slate-400 tracking-widest uppercase">Al Jamiathul Kauzariyya</span>
                </div>
            </div>
            
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs">
                    <span class="w-2 h-2 rounded-full bg-emerald-400"></span>
                    <span class="text-slate-350">Logged in as **Coordinator**</span>
                </div>
                <a href="index.php" class="text-sm font-medium text-slate-400 hover:text-white transition">Home</a>
            </div>
        </div>
    </nav>
    ...
    <main class="max-w-7xl mx-auto px-6 py-12 space-y-8">
        ...
        <!-- Tab Bar -->
        <div class="flex flex-wrap gap-2 p-1.5 rounded-2xl bg-white/5 border border-white/10 max-w-2xl">
            <button onclick="switchTab('overview')" id="btn-overview" class="tab-btn active flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition border border-transparent">
                <i class="fa-solid fa-chart-pie"></i> Overview
            </button>
            <button onclick="switchTab('courses')" id="btn-courses" class="tab-btn flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition border border-transparent">
                <i class="fa-solid fa-graduation-cap"></i> Courses & Classes
            </button>
            <button onclick="switchTab('teachers')" id="btn-teachers" class="tab-btn flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition border border-transparent">
                <i class="fa-solid fa-chalkboard-user"></i> Teachers
            </button>
            <button onclick="switchTab('supervisors')" id="btn-supervisors" class="tab-btn flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition border border-transparent">
                <i class="fa-solid fa-user-tie"></i> Supervisors
            </button>
            <button onclick="switchTab('students')" id="btn-students" class="tab-btn flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition border border-transparent">
        ...
        <section id="tab-overview" class="tab-content space-y-8">
            ...
            <div class="glass-panel rounded-3xl p-8 md:p-12 space-y-4">
                <h2 class="text-3xl font-bold bg-gradient-to-r from-emerald-400 to-blue-400 bg-clip-text text-transparent">Hello, Coordinator!</h2>
                <p class="text-slate-300 max-w-3xl leading-relaxed">
                    This is your dashboard to oversee the entire educational architecture of Makthab Kauzariyya. You can navigate through the tabs to map courses, track supervisors, assign classes to teachers, and manage the student directory.
                </p>
                <div class="flex gap-4 pt-2">
                    <button onclick="switchTab('courses')" class="bg-gradient-to-r from-emerald-400 to-blue-500 text-slate-950 px-6 py-3 rounded-xl font-bold hover:shadow-lg hover:shadow-emerald-400/20 transition">
        ...
        <section id="tab-courses" class="tab-content hidden space-y-8">
            <div class="flex justify-between items-center">
                <div>
                    <h2 class="text-2xl font-bold text-white">Courses & Classes Tree</h2>
                    <p class="text-sm text-slate-450">View courses and drill down into classes, students, and supervisors.</p>
                </div>
            </div>
                ...
                <?php foreach ($courses as $course): ?>
                    <div class="glass-panel rounded-2xl p-6 space-y-4">
                        <div class="flex justify-between items-start border-b border-white/10 pb-4">
                            ...
                            <p class="text-sm text-slate-400 max-w-md text-right"><?= htmlspecialchars($course['description']) ?></p>
                        </div>
        ...
        <section id="tab-teachers" class="tab-content hidden space-y-8">
            <div>
                <h2 class="text-2xl font-bold text-white">Teachers Registry</h2>
                <p class="text-sm text-slate-450">Monitor teachers and the multiple classes they teach.</p>
            </div>
        ...
        <section id="tab-supervisors" class="tab-content hidden space-y-8">
            <div>
                <h2 class="text-2xl font-bold text-white">Supervisors Registry</h2>
                <p class="text-sm text-slate-450">Monitor supervisors and the student classes they oversee.</p>
            </div>
        ...
        <section id="tab-students" class="tab-content hidden space-y-8">
            <div>
                ...
                <p class="text-sm text-slate-450">Direct search and list of all enrolled students.</p>
            </div>
